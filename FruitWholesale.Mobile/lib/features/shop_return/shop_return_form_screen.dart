...
class ShopReturnFormScreen extends StatefulWidget {
  /// Null means "create new"; otherwise the existing return being edited.
  final int? shopReturnId;

  const ShopReturnFormScreen({super.key, this.shopReturnId});

  bool get isEditing => shopReturnId != null;

  @override
  State<ShopReturnFormScreen> createState() => _ShopReturnFormScreenState();
}
...
class _ShopReturnFormScreenState extends State<ShopReturnFormScreen> {
  late final ShopReturnService _shopReturnService = ShopReturnService(context.read<ApiClient>());
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (widget.isEditing) {
        ...
        _referenceNoController.text = await _shopReturnService.getNextReferenceNo();
      }
  ...
  Future<void> _save() async {
    ...
    final shopReturn = ShopReturnDetail(
      returnDate: _date,
      shopId: _selectedShopId!,
      referenceNo: _referenceNoController.text.trim(),
      remarks: _remarksController.text.trim().isEmpty ? null : _remarksController.text.trim(),
      items: validItems
          .map((i) => ShopReturnItem(fruitId: i.fruitId!, quantity: i.quantity, unitPrice: i.unitPrice))
          .toList(),
    );

    try {
      if (widget.isEditing) {
        await _shopReturnService.update(widget.shopReturnId!, shopReturn);
      } else {
        await _shopReturnService.create(shopReturn);
      }
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.isEditing ? 'Edit Shop Return' : 'New Shop Return')),
      body: _loading
