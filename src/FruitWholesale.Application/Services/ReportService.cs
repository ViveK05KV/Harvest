...
public class ReportService(IReportRepository repository) : IReportService
{
    ...
    {
        var expensesTask = repository.GetDailyExpenseAsync(fromDate, toDate, expenseCategoryId);

        // A specific expense category was requested - salary isn't one of those
        // categories, so it stays out when the list is filtered down to one.
        if (expenseCategoryId != null)
        {
            return await expensesTask;
        }

        var salaryTask = repository.GetDailySalaryAsync(fromDate, toDate);
        await Task.WhenAll(expensesTask, salaryTask);

        return (await expensesTask)
            .Concat(await salaryTask)
            .OrderBy(r => r.ExpenseDate)
            .ToList();
    }
